...
static const char arg_hexlisting[]	 = "HEX listing filename";
// long options
...
#define OPTION_HEXLISTING	   "hexlisting"
...
// variables
static const char		 *
	   *toplevel_sources_plat; // source file names given on command line (platform-style)
...
{
	puts( "This is ACME, release " RELEASE " (\"" CODENAME "\"), " CHANGE_DATE " " CHANGE_YEAR
		  "\n"
		  "  " PLATFORM_VERSION );
...
static void show_help_and_exit( void )
{
	...
	puts(
		...
		"  -f, --" OPTION_FORMAT
		" FORMAT       set output file format\n"
		"  -o, --" OPTION_OUTFILE
		" FILE        set output file name\n"
		"  -r, --" OPTION_REPORT
		" FILE         set report file name\n"
		"  -l, --" OPTION_SYMBOLLIST
		" FILE     set symbol list file name\n"
		"      --" OPTION_LABELDUMP "           (old name for --" OPTION_SYMBOLLIST
		")\n"
		"      --" OPTION_VICELABELS
		" FILE     set file name for label dump in VICE format\n"
		"      --" OPTION_HEXLISTING
		" FILE     set file name for a hex listing\n"
		"      --" OPTION_SETPC
		" VALUE         set program counter\n"
		"      --" OPTION_FROM_TO
		" VALUE VALUE set start and end+1 of output file\n"
		"      --" OPTION_CPU
		" CPU             set target processor\n"
		"      --" OPTION_INITMEM
		" VALUE       define 'empty' memory\n"
		"      --" OPTION_MAXERRORS
		" NUMBER    set number of errors before exiting\n"
		"      --" OPTION_MAXDEPTH
		" NUMBER     set recursion depth for macro calls and !src\n"
		"      --" OPTION_IGNORE_ZEROES
		"       do not determine number size by leading zeroes\n"
		"      --" OPTION_STRICT_SEGMENTS
		...
		"      --" OPTION_FULLSTOP
		"            use '.' as pseudo opcode prefix\n"
		"      --" OPTION_DIALECT
		" VERSION     behave like different version\n"
		"      --" OPTION_DEBUGLEVEL
		" VALUE    drop all higher-level debug messages\n"
...
int ACME_finalize( int exit_code )
{
	...
	if ( config.symbollist_filename )
	{
		...
		fd = fopen(
			config.symbollist_filename,
			FILE_WRITETEXT ); // FIXME - what if filename is given via !sl in sub-dir? fix path!
		if ( fd )
		{
			symbols_list( fd );
			fclose( fd );
	...
	if ( config.hexlisting_filename )
	{
		// output of hex listing:
		// -> start with a sanity check: can we get data, is there anything to output?
		intval_t	amount, loadaddr;
		const char *body;
		output_get_result( &body, &amount, &loadaddr );
		if ( body && amount )
		{
			// show filename _now_ because I do not want to massage it into perror()
			printf( "Writing HEX listing file \"%s\".\n", config.hexlisting_filename );
			fd = fopen( config.hexlisting_filename, FILE_WRITETEXT );
			if ( fd )
			{
				const char *fn = toplevel_sources_plat[ 0 ];
				int			i  = 0;
				// find last filename part
				while ( fn[ i ] )
					if ( ( fn[ i ] == '/' || fn[ i ] == '\\' ) && fn[ i + 1 ] ) fn += i + 1, i = 0;
					else ++i;
				fprintf( fd,
						 "/*\n\tACME HEX listing of \"%s\""
						 "\n\tload_address: 0x%04x\n*/",
						 fn, loadaddr );
				i = -1;
				while ( ++i < amount )
				...
				perror( "Error: Cannot open HEX listing file" );
				exit_code = EXIT_FAILURE;
			}
		}
	}
...
static void save_output_file( void )
{
	...
	switch ( config.outfile_format )
	{
		...
		case OUTFILE_FORMAT_APPLE:
			...
			break;
		default: BUG( "IllegalOutformat1", config.outfile_format );
	}

	// open file
	fd = fopen(
		config.output_filename,
		FILE_WRITEBINARY ); // FIXME - what if filename is given via !to in sub-dir? fix path!
	if ( fd == NULL )
	{
	...
	{
		case OUTFILE_FORMAT_UNSPECIFIED:
		case OUTFILE_FORMAT_PLAIN: PLATFORM_SETFILETYPE_PLAIN( config.output_filename ); break;
		case OUTFILE_FORMAT_APPLE: PLATFORM_SETFILETYPE_APPLE( config.output_filename ); break;
		case OUTFILE_FORMAT_CBM: PLATFORM_SETFILETYPE_CBM( config.output_filename ); break;
		default: BUG( "IllegalOutformat2", config.outfile_format );
	}
}
...
static void perform_pass( void )
{
	...
	for ( ii = 0; ii < toplevel_src_count; ++ii )
	{
		if ( ( fd = fopen( toplevel_sources_plat[ ii ], FILE_READBINARY ) ) )
		{
			...
			if ( toplevel_sources_plat[ ii ][ 0 ] == '-' )
				fprintf( stderr,
						 "Options (starting with '-') must be given _before_ source files!\n" );
			++pass.error_count;
		}
	}
	...
	if ( config.process_verbosity >= 8 )
		printf( "Undefined expressions: %d. Symbol updates: %d.\n", pass.undefined_count,
				pass.changed_count );
	if ( pass.error_count ) exit( ACME_finalize( EXIT_FAILURE ) );
...
static void			 do_actual_work( void )
{
			...
			|| pass.changed_count )
	{
		...
		if ( --sanity.passes_left < 0 )
		{
			...
			// ...or maybe do one additional pass where all errors are reported, including "not
			// defined" and "value has changed".
	...
	{ // FIXME - use pass.needvalue_count instead!
		...
		if ( config.process_verbosity >= 2 ) puts( "Extra pass to display errors." );
...
static void set_output_format( const char format_name[] )
{
	...
	if ( format_name )
	{
		...
		if ( config.outfile_format != OUTFILE_FORMAT_UNSPECIFIED ) return; // ok
...
static void define_symbol( const char definition[] )
{
	...
	dynabuf_clear( GlobalDynaBuf );
	while ( ( *walk != '=' ) && ( *walk != '\0' ) ) dynabuf_append( GlobalDynaBuf, *walk++ );
	if ( ( *walk == '\0' ) || ( walk[ 1 ] == '\0' ) ) could_not_parse( definition );
...
struct dialect_info dialects[] = {
	...
	//	{V0_93__SHORTER_SETPC_WARNING,	"0.93",		"\"*=\" in offset assembly gives shorter warning
	// but still switches off"},
	...
	{ V0_94_8__DISABLED_OBSOLETE, "0.94.8",
	  "\"*=\" works inside \"!pseudopc\", disabled \"!cbm/!realpc/!subzone\"" },
...
{
	if ( strcmp( string, OPTION_HELP ) == 0 ) show_help_and_exit();
	...
	else if ( strcmp( string, OPTION_HEXLISTING ) == 0 )
		config.hexlisting_filename = cliargs_safe_get_next( arg_hexlisting );
	...
	{
		config.outfile_start =
			string_to_nonneg_number( cliargs_safe_get_next( "start address of output file" ) );
		config.outfile_limit =
			string_to_nonneg_number( cliargs_safe_get_next( "end+1 of output file" ) );
	...
	else if ( strcmp( string, OPTION_USE_STDOUT ) == 0 ) config.msg_stream = stdout;
	else if ( strcmp( string, OPTION_MSVC ) == 0 ) config.format_msvc = TRUE;
	else if ( strcmp( string, OPTION_FULLSTOP ) == 0 ) config.pseudoop_prefix = '.';
	else if ( strcmp( string, OPTION_IGNORE_ZEROES ) == 0 ) config.honor_leading_zeroes = FALSE;
	else if ( strcmp( string, OPTION_STRICT_SEGMENTS ) == 0 )
		config.debuglevel_segmentprobs = DEBUGLEVEL_ERROR;
	else if ( strcmp( string, OPTION_STRICT ) == 0 ) config.all_warnings_are_errors = TRUE;
	...
	else if ( strcmp( string, OPTION_COLOR ) == 0 ) config.format_color = TRUE;
	else if ( strcmp( string, OPTION_VERSION ) == 0 ) show_version( TRUE ); else return string;
	return NULL;
}
	...
	while ( *argument )
	{
		switch ( *argument )
		{
			...
			case 'I': // "-I" adds an include directory
				if ( argument[ 1 ] ) includepaths_add( argument + 1 );
				else includepaths_add( cliargs_safe_get_next( "include path" ) );
				goto done;
...
int main( int argc, const char *argv[] )
{
	...
	{
		fprintf( stderr,
				 "Error: typedefs for intval_t and uintval_t must use types with at least 32 "
				 "bits.\nPlease edit config.h and recompile.\n" );
		exit( EXIT_FAILURE );
	}
	config_default( &config );
	// if called without any arguments, show usage info (not full help)
	if ( argc == 1 ) show_help_and_exit();
	...
	if ( ( config.outfile_start != NO_VALUE_GIVEN )
		 && ( config.outfile_start >= config.outbuf_size ) )
	{
	...
	if ( ( config.outfile_limit != NO_VALUE_GIVEN )
		 && ( config.outfile_limit > config.outbuf_size ) )
	{
		fprintf( stderr, "%sEnd+1 of output file exceeds outbuffer size.\n", cliargs_error );
		exit( EXIT_FAILURE );
	}
	if ( ( config.outfile_start != NO_VALUE_GIVEN ) && ( config.outfile_limit != NO_VALUE_GIVEN )
