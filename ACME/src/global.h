...
#define BYTE_STARTS_KEYWORD( b )                                                                   \
	( global_byte_flags[ ( unsigned char ) b ]                                                     \
	  & ( 1u << 7 ) ) // byte is allowed at start of keyword (a-z, A-Z, _, everything>127)
#define BYTE_CONTINUES_KEYWORD( b )                                                                \
	( global_byte_flags[ ( unsigned char ) b ]                                                     \
	  & ( 1u << 6 ) ) // byte is allowed in a keyword (as above, plus digits)
// #define BYTE_TO_LOWER_CASE(b)	bit 5 means: "byte is upper case, and can be converted to lower
// case by ORing this bit" - but this is not used at the moment!
#define BYTE_IS_SYNTAX_CHAR( b )                                                                   \
	( global_byte_flags[ ( unsigned char ) b ] & ( 1u << 4 ) ) // special character for input syntax
#define BYTE_FOLLOWS_ANON( b )                                                                     \
	( global_byte_flags[ ( unsigned char ) b ] & ( 1u << 3 ) ) // preceding '-' are backward label
...
enum dialect {
	V0_85__OLDEST_SUPPORTED,	  // v0.85 looks like the oldest version it makes sense to actually
								  // support
	V0_86__DEPRECATE_REALPC,	  // v0.86 made !pseudopc/!realpc give a warning to use !pseudopc{}
								  // instead, and !to wants a file format
	V0_93__SHORTER_SETPC_WARNING, // v0.93 claimed to allow *= inside !pseudopc blocks, but didn't.
								  // It shortened the warning, but '}' or !realpc clobbered PC
	V0_94_6__RIGHT_ASSOC_POWER,	  // v0.94.6 made "power of" operator right-associative
								// v0.94.7 fixed a bug: empty code segments no longer included in
								// output file
	V0_94_8__DISABLED_OBSOLETE,	   // v0.94.8 made *= work inside !pseudopc, disabled
								   // !cbm/!realpc/!subzone
...
struct config
{
	...
	const char			  *hexlisting_filename;
...
struct pass
{
	...
	int undefined_count; // counts undefined expression results (if this stops decreasing, next pass
						 // must list them as errors)
	// int	needvalue_count;	// counts undefined expression results actually needed for output
	// (when this hits zero, we're done)	FIXME - use
